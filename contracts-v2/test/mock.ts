...
Assertion.addMethod("callMocksDebug", async function (...calls: MockCall[]): Promise<Transaction> {
  ...
  for (let i = 0; i < calls.length; i++) {
    ...
    try {
      ...
      await txBuilder();
      throw new Error("The transaction succeeds while some mock function is not called as expected");
    }
    if (rets === undefined) {
      await func.returns();
    } else {
  ...
  console.warn("WARNING: 'callMocksDebug' is only for debugging. Use 'callMocks' instead after you make things right.");
  return tx;
});
